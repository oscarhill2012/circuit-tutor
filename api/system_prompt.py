...
def build_first_user_message(req: TutorRequest) -> dict[str, str]:
    """Build the slim user message dict. Callers append it straight into the messages list."""
    payload = {
...
def build_corrective_message(reason: str, detail: str) -> dict[str, str]:
    """System message injected when the post-validator rejects a proposed reply.

    Re-invokes the model once with the rejection reason; the agent loop falls back
    if rejected again.
    """
    return {
        "role": "system",
