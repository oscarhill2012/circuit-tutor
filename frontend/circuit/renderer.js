...
let previewG = null;
let wireBarsG = null;
// Dev-mode diagnostic layer for visualising wire overlaps (see renderOverlapsDebug).
let overlapsG = null;
...
export function initRenderer() {
  ...
  // Overlap debug layer: sits above wires, below components, so red bands
  // hide the wire stroke beneath but do not obscure component bodies/labels.
...
export function render() {
  ...
  // Paint collinear sub-segments shared by two wires (dev mode only).
...
// Diagnostic only. Gated on dev mode so the O(N^2) segment-pair
