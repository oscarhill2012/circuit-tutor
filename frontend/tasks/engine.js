// Task engine: loads the ordered task list from frontend/data/tasks.json
...
export async function loadTasks() {
  const res = await fetch('./frontend/data/tasks.json', { cache: 'no-cache' });
...
export function startTask(taskId) {
  ...
  // All task types are driven from Professor Volt's panel — the floating
  // top-of-canvas widget is hidden. The widget DOM stays because the canvas
  // tooling targets the canvas-wrap layer, but renders empty.
  hideTaskWidget();
  renderTask();
  introduceTask(t);
  notifyActiveTaskChange();
...
export function renderTask() {
  ...
  if (t.type === 'measure') {
    // Measure tasks are driven from Professor Volt's panel \u2014 the student
    // types their reading in chat and clicks "Check my circuit". The widget renders nothing.
    card.innerHTML = '';
    ...
    // Exploration tasks use Check-my-circuit as "I'm done exploring". Widget hidden.
    card.innerHTML = '';
  }
  if (done) {
...
// Scenario tasks: full LLM circuit verdict. Measure tasks: local-deterministic
// reading check merged with the LLM verdict. Exploration tasks: complete immediately.
// Returns the final verdict so callers can react; the tutor reply is appended to
// chat by askTutorCheckScenario when the backend is involved.
