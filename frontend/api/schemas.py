"""Pydantic models — single source of truth for the v2 tutor agent contract.

These shapes are imported by tools.py, agent_runner.py, tool_dispatch.py, and
the post-validator. The reply envelope mirrors the field set the legacy client
already consumes (see frontend/src/ui/tutorPanel.js), so the UI does not need
to change when the agent loop ships.

Plan ref: tutor-redo/01-schemas-and-tools.md §1.1, §3.2 of the full plan.
"""
...
class KnowledgeEntry(BaseModel):
    ...
    `requires_citation` and `latex` are added in Chunk 2 with safe defaults so
    Chunk 1 readers (which do not yet emit them) round-trip cleanly.
    """
...
class AnalyseTopologyReturn(BaseModel):
    ...
    `analysis` is the existing dict — we keep it loose (Any) so the validator
    output can evolve without breaking schemas. The agent should treat
    `suggested_focus` as advisory, not authoritative (full plan §3.6).
    """
...
# ---------------------------------------------------------------------------
# Inbound request from the client (the slim payload from full plan §7)
# ---------------------------------------------------------------------------
...
class TutorRequest(BaseModel):
    ...
    debug: bool = False
    # In Chunk 3 the circuit snapshot still rides on the wire so the oracle
    # tools can read it; the slim wire is finalised in Chunk 5.
...
# ---------------------------------------------------------------------------
# Ledger entries (Chunk 1 fixes the shape; Chunk 3 builds the agent loop
# that populates them per turn)
# ---------------------------------------------------------------------------
