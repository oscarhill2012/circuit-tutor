"""Nine pure-server tool implementations for the v2 tutor agent.
...
pure: no module-level state mutation, no globals beyond the loaded KB. This
makes them trivially unit-testable without a model in the loop.

The agent loop (Chunk 3) glues these into a tool-call dispatcher; this file
is just the catalogue. The post-validator (Chunk 2) reads the ledger of
calls these tools record, so the args/return shapes are load-bearing.

Plan ref: tutor-redo/01-schemas-and-tools.md §1.4 + full plan §3.2.
"""
...
# ---------------------------------------------------------------------------
# Embeddings sidecar (Chunk 2). Loaded best-effort at import — if the file is
# missing or malformed, the bag-of-words path stays the primary route.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Lookup ledger (per-turn). The dispatcher (Chunk 3) creates one of these
# per request so cite_fact() can validate against this turn's lookups.
# ---------------------------------------------------------------------------
...
class LookupLedger:
    ...
    The post-validator (Chunk 2) and the cite_fact tool both consult this so
    a model cannot cite a fact it never retrieved.
    """
...
# ---------------------------------------------------------------------------
# Bag-of-words retrieval (lifted from tutor.py:_retrieve so we don't depend
# on a file we will delete in Chunk 6). Chunk 2 swaps this for embeddings.
# ---------------------------------------------------------------------------
...
def lookup_knowledge(
    ...
    *,
    ...
    can later verify a citation was actually retrieved (full plan §5).
    """
...
def _extract_state_for_analysis(circuit_state: dict[str, Any]) -> dict[str, Any]:
    """Translate the frontend's snapshot into circuit_validator.analyse()'s shape.
...
def _suggested_focus(analysis: dict[str, Any]) -> dict[str, Any] | None:
    ...
    Logic lifted from tutor.py:_triage_focus. The crucial behavioural change
    versus the legacy: this is returned to the model as advisory context.
    The model is free to ignore it (e.g. when the student is deliberately
    exploring a wrong configuration).

    Plan ref: full plan §3.6.
    """
...
def inspect_circuit(
    ...
    *,
    ...
    The legacy circuitSnapshot() stripped voltage/resistance unless the
    student message had "numeric intent"; that lied to the model
    (full plan C5). The new contract: ask for fields, get them.
    """
...
def mark_target(
    ...
    *,
    ...
    The legacy applyVisualInstructions() blindly applied whatever the model
    said, allowing invented ids like "L7" to silently no-op. The new contract:
    the server rejects any id not present in `circuit_state.components`
    (full plan C1 fix).
    """
...
def update_session_state(
    ...
    *,
    ...
    Rejects unknown KB-misconception ids (full plan C4 / §4 point 3) so the
    model cannot invent misconception names that the rest of the system
    relies on.
    """
...
def validate_task(
    ...
    *,
    ...
    The model writes the prose; this tool writes the verdict (full plan M5
    fix). For Chunk 1 we surface the data the legacy SCENARIO_PROMPT_SUFFIX
    was asking the model to look at — the actual check_request payload is
    server-authoritative for `reading_status`.
    """
...
# ---------------------------------------------------------------------------
# Convenience: a name->callable map for Chunk 3's dispatcher.
# ---------------------------------------------------------------------------
